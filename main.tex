...
To answer these questions, we explore 8 strategies (described in Table~\ref{tab:strategies}) and optimise ES with a timber production target of 35~Mm$^3$yr$^{-1}$ (equivalent to the current timber production in Amazonia \cite{Lentini2005}).

We then combine all three ES into a single optimisation framework and analyse how provision of each ES is affected by prioritisation of management objectives. Finally, we explored 8 strategies and their consequences on ES provision, depending on total timber production (see Table~\ref{tab:strategies} for strategy description).
